...
function updateProd() {
    ...
    while ($i < sizeof($styles)) {
        if ($styles[$i] != "") {
            ...
            $result = $obj->updateProd($plant_no, $date, $session, $line_nos[$i], $styles[$i], $start, $outputs[$i], $accepted[$i], $rejected[$i], $rej_rep[$i], $mos[$i], $remarks[$i]);
...
function hoursUpdate() {
    ...
    include '../model/production.php';
    $obj = new Production();
    foreach($plant_no as $p){
        $obj -> hoursUpdate($date,$p,$daytype,$wh,$ot);
    }
