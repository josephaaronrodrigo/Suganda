...
class Production {
    ...
    function getAllStyleOrders() {
        $conn = new Connection();
        $sql = "SELECT DISTINCT style FROM style_allocation";
        $result = $conn->query($sql);
        $i = 0;
        $array = array();
        while ($row = mysql_fetch_array($result)) {
            $s = substr($row['style'], 0, (strlen($row['style']) - 2));
            if (!in_array($s, $array)) {
                $array[$i] = $s;
                $i++;
            }
    ...
    function getCompleted($style) {
        $conn = new Connection();
        $sql = "SELECT SUM(output) AS completed FROM production_out WHERE style LIKE '$style-_'";
        $result = $conn->query($sql);
        $array = mysql_fetch_array($result);
    ...
    function getAttTotal($date, $type, $plant_no) {
        $conn = new Connection();
        $sql = "SELECT SUM($type) AS total FROM attendance WHERE date LIKE '$date' AND plant_no = '$plant_no' AND emp_type NOT LIKE 'Pregnant Women'";
        $result = $conn->query($sql);
        return $result;
    }
    ...
    function updateHourPlan($date, $ot_h, $working_h, $plant_no) {
        ...
        $result = $conn->query($sql);
        if (mysql_num_rows($result) == 0) {
            $sql1 = "INSERT INTO ot_plan(plant_no,date,working_h,ot_h) VALUES('$plant_no','$date','$working_h','$ot_h')";
            $conn->query($sql1);
        }
        return;
    }
    ...
    function genStyleAllDet($column, $style) {
        $conn = new Connection();
        $sql = "SELECT SUM($column) AS total FROM style_allocation WHERE style LIKE '$style-_'";
        $result = $conn->query($sql);
        $array = mysql_fetch_array($result);
        $output = $array['total'];
        return $output;
    ...
    function hoursUpdate($date, $plant_no, $daytype, $wh, $ot) {
        $conn = new Connection();
        $sql = "SELECT * FROM ot_plan WHERE date LIKE '$date' AND plant_no LIKE '$plant_no'";
        $result = $conn->query($sql);
        if (mysql_num_rows($result) > 0) {
            $sql1 = "UPDATE ot_plan SET working_h='$wh',ot_h='$ot',day_type='$daytype' WHERE date LIKE '$date' AND plant_no LIKE '$plant_no'";
        } else {
            $sql1 = "INSERT INTO ot_plan VALUES('$plant_no','$date','$daytype','$wh','$ot')";
        }
        $conn->query($sql1);
    }

    function getDayType($plant_no, $date) {
        $conn = new Connection();
        $sql = "SELECT day_type FROM ot_plan WHERE date LIKE '$date' AND plant_no LIKE '$plant_no'";
        $result = $conn->query($sql);
        if (mysql_num_rows($result) > 0) {
            $array = mysql_fetch_array($result);
            $daytype = $array['day_type'];
        } else {
            $daytype = NULL;
        }
        return $daytype;
    ...
    function plannedMOS($date, $plant_no) {
        $conn = new Connection();
        $sql = "SELECT * FROM month_plan WHERE date LIKE '$date' AND plant_no LIKE '$plant_no'";
        $result = $conn->query($sql);
        $plan = 0;
        if (mysql_num_rows($result) > 0) {
            while ($row = mysql_fetch_array($result)) {
                $style = $row['style'];
                $sql1 = "SELECT rqd_mos FROM style_allocation WHERE start_date <= '$date' AND end_date >= '$date' AND style LIKE '$style' AND plant_no LIKE '$plant_no'";
                $result1 = $conn->query($sql1);
    ...
    function totActProd($plant_no, $style, $start_date, $end_date, $date, $line) {
        $conn = new Connection();
        $sql = "SELECT SUM(output) AS completed FROM production_out WHERE style = '$style' AND date >= '$start_date' AND date < '$end_date' AND date <= '$date' AND plant_no = '$plant_no' AND line_no LIKE '$line'";
        $result = $conn->query($sql);
        return $result;
    }
    
    function countWorkingDays($plant_no,$sdate,$edate){
        $conn = new Connection();
        $sql = "SELECT COUNT(date) AS days FROM ot_plan WHERE date >= '$sdate' AND date <= '$edate' AND plant_no = '$plant_no' AND day_type IN ('Normal Work','Half Day','Cover Day')";
        $result = $conn ->query($sql);
        $array = mysql_fetch_array($result);
        $days = $array['days'];
        return $days;
